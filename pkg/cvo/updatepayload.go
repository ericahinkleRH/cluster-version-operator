...
// the payload using the specified desired version. When the requested version cannot be
// resolved it also returns an error describing why, so the caller can report it in status.
func findUpdateFromConfig(config *configv1.ClusterVersion, currentArch configv1.ClusterVersionArchitecture) (configv1.Update, bool, error) {
	...
	if len(update.Image) == 0 {
		if desiredUpdateAlreadyAchieved(config, update, currentArch) {
			return configv1.Update{}, false, nil
		}
...
func desiredUpdateAlreadyAchieved(config *configv1.ClusterVersion, update *configv1.Update, currentArch configv1.ClusterVersionArchitecture) bool {
	if len(update.Version) == 0 {
		return true
	}
	if update.Version != config.Status.Desired.Version {
		return false
	}
	return len(update.Architecture) == 0 || update.Architecture == currentArch
}

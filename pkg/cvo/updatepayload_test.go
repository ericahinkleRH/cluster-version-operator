...
func Test_findUpdateFromConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantMessage: `No release with version "4.15.0" in status.availableUpdates; set spec.desiredUpdate.image to request it explicitly`,
		},
		{
		...
		{
			name: "completed single-to-multi transition is already achieved",
			config: &configv1.ClusterVersion{
				Spec: configv1.ClusterVersionSpec{
					DesiredUpdate: &configv1.Update{
						Architecture: configv1.ClusterVersionArchitectureMulti,
		...
		{
			name: "accept-risks-only request does not select a release",
			config: &configv1.ClusterVersion{
				Spec: configv1.ClusterVersionSpec{
					DesiredUpdate: &configv1.Update{
						AcceptRisks: []configv1.AcceptRisk{{Name: "RiskA"}},
					},
				},
			},
			currentArch: "amd64",
		...
		},
		{
			...
			wantFound:   false,
			wantErr:     "DesiredReleaseUnavailable",
			wantMessage: `No release with version "4.16.0" in status.availableUpdates; set spec.desiredUpdate.image to request it explicitly`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr != "" {
				...
				if updateErr.Message != tt.wantMessage {
					t.Errorf("error message = %q, want %q", updateErr.Message, tt.wantMessage)
				}
			} else if gotErr != nil {
